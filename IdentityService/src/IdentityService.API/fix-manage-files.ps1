# Get all Manage folder .cshtml.cs files with only 3 lines (malformed files)
$files = Get-ChildItem -Path ".\Areas\Identity\Pages\Account\Manage" -Filter "*.cshtml.cs" | 
         Where-Object { 
             $content = Get-Content $_.FullName
             $content.Count -le 3 -and $content -match "ApplicationUser"
         }

Write-Host "Found $($files.Count) files to fix in Manage folder"
...
Write-Host "Finished fixing files in Manage folder" 